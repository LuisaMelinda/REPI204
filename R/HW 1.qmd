...
## 1 Maximum likelihood inference for Bernoulli outcomes
...
The asymptotic p-value for the null hypothesis was  0.0001139833
...
where:
$$\sum_{i=1}^{n} x_i = n\pi $$
Under null hypothesis: $$\sum_{i=1}^{n} x_i = 100\cdot0.5 = 50 $$
...
compute_power_binomial <- function(pi_hat, sample_size, pi_null = 0.5) {
  ...
  power <- pbinom(qbinom(0.025, size = sample_size, prob = pi_null), size = sample_size, prob = pi_hat) +
           (1 - pbinom(qbinom(0.975, size = sample_size, prob = pi_null), size = sample_size, prob = pi_hat))
...
power <- compute_power_binomial(pi_hat, sample_size)
power
